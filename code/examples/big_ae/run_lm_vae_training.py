        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            epoch_iterator.set_description(
                (
                    ...
                    f"beta: {model_vae.module.args.beta if hasattr(model_vae, 'module') else model_vae.args.beta:.3f}"
                )
            )
...
def evaluate(
    ...
    result = {"perplexity": ppl, "elbo": elbo, "kl": kl, "nll": nll, "au": au, "mi": mi}
...
def evaluate_rec(
    ...
    result = {"rec_w": nll_w, "rec_s": nll_s}
...
def main():
    ...
    if args.save_bert_gpt_init:
        encoder_path = os.path.join(
            args.output_dir, f"initial-models-tokenization-enoder-{args.latent_size}"
        )
        ...
        decoder_path = os.path.join(
            args.output_dir, f"initial-models-tokenization-decoder-{args.latent_size}"
        )
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        global_step = (
            checkpoint_dir[-1].split("-")[-1] if len(checkpoint_dir) > 1 else ""
        )
    ...
    if args.do_eval_rec and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model_encoder = encoder_model_class.from_pretrained(
                checkpoint[0], latent_size=args.latent_size
            )
            model_encoder.to(args.device)
            ...
            result = dict(
                (k + "_test_{}".format(global_step), v) for k, v in result.items()
            )
            results.update(result)
            ...
            result = dict(
                (k + "_train_{}".format(global_step), v) for k, v in result.items()
            )
            results.update(result)
