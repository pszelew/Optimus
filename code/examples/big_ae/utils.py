...
class FeatureDataset(Dataset):
    ...
    def collate(features):
        input_ids_bert = pad_sequence(
            [torch.tensor(f.input_ids_bert, dtype=torch.long) for f in features],
            batch_first=True,
            padding_value=0,
        )
        input_ids_gpt = pad_sequence(
            [torch.tensor(f.input_ids_gpt, dtype=torch.long) for f in features],
            batch_first=True,
            padding_value=0,
        )
...
class Dialog_BucketingDataLoader(object):
    def __init__(
        ...
        self.dataset = Dialog_TokenDataset(
            tokenizer, args, file_path, block_size=args.block_size
        )
        self.batch_size = batch_size
        self.max_len = max_seq_length
    ...
    def __iter__(self):
        ...
        loader = DataLoader(
            ...
            batch_sampler=sampler,
            num_workers=0,
            collate_fn=Dialog_TokenDataset.collate,
        )
        yield from loader
...
class MultipleFiles_DataLoader(object):
    def __init__(
        ...
        self.dataset = PreparedTokenDataset(
            tokenizer,
            self.args,
    ...
    def __iter__(self):
        ...
        loader = DataLoader(
            ...
            if self.use_tensor
            else PreparedTokenDataset.get_examples,
        )
        yield from loader
        ...
        self.cached_features_file = os.path.join(
            ...
            + f".segmented.nltk.split.seq64.{self.file_idx}.json",
        )
        self.dataset = PreparedTokenDataset(
            self.tokenizer,
            self.args,
...
class PreparedTokenDataset(Dataset):
    def __init__(
        ...
        if (
            args.dataset == "Yahoo"
            or args.dataset == "Penn"
            or args.dataset == "Snli"
            or args.dataset == "Debug"
            or args.dataset == "wikipedia"
        ):
            label_on = False
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size, args):
        ...
        with open(fname) as fin:
            for line in fin:
                ...
                tokenized_text1 = self.tokenizers[1].add_special_tokens_single_sentence(
                    tokenized_text1
                )
                tokenized_text1 = (
...
class Dialog_TokenDataset(Dataset):
    def __init__(
        ...
        if os.path.exists(cached_features_file):
            ...
            dropped, count = self._read_dialog_corpus_natural_split(
                fname=file_path,
                label=label_on,
                max_length=block_size,
                block_size=block_size,
    ...
    def collate(examples):
        # Convert to Tensors and build dataset
        input_ids_bert_ctx = pad_sequence(
            [torch.tensor(f["bert_token_ctx"], dtype=torch.long) for f in examples],
            batch_first=True,
            padding_value=bert_pad_token,
        )
        input_ids_bert = pad_sequence(
    ...
    def _read_dialog_corpus_natural_split(
        ...
        with open(fname) as fin:
            for line in fin:
                ...
                tokenized_text_ctx = self.tokenizers[0].convert_tokens_to_ids(
                    self.tokenizers[0].tokenize(context_text)
                )
                tokenized_text_ctx = self.tokenizers[
                    0
                ].add_special_tokens_single_sentence(tokenized_text_ctx)
                ...
                tokenized_text0 = self.tokenizers[0].convert_tokens_to_ids(
                    self.tokenizers[0].tokenize(response_text)
                )
                tokenized_text0 = self.tokenizers[0].add_special_tokens_single_sentence(
                    tokenized_text0
                )
                ...
                tokenized_text1 = self.tokenizers[1].convert_tokens_to_ids(
                    self.tokenizers[1].tokenize(response_text)
                )
                tokenized_text1 = self.tokenizers[1].add_special_tokens_single_sentence(
                    tokenized_text1
                )
...
class TextDataset_Split(Dataset):
    def __init__(
        ...
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(
            directory, f"cached_lm_gpt_{block_size}_{filename}"
        )
        ...
        if os.path.exists(cached_features_file):
            ...
            if text_split_mode == "block":
                ...
                self._read_corpus_natural_split(
                    fname=file_path,
                    label=label_on,
                    max_length=block_size,
                    block_size=block_size,
    ...
    def __getitem__(self, item):
        ...
        tokenized_text_lengths = torch.tensor(
            [self.examples[item][1]], dtype=torch.long
        )
        # pdb.set_trace()
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size):
        ...
        with open(fname) as fin:
            for line in fin:
                ...
                tokenized_text1 = self.tokenizer.convert_tokens_to_ids(
                    self.tokenizer.tokenize(split_line_text)
                )
                tokenized_text1 = self.tokenizer.add_special_tokens_single_sentence(
                    tokenized_text1
                )
                tokenized_text1_length = len(tokenized_text1)

                tokenized_text1 = (
                    [self.bos_token_id] + tokenized_text1 + [self.eos_token_id]
                )
                tokenized_text1 = tokenized_text1 + (
                    [self.pad_token_id] * (block_size - tokenized_text1_length - 2)
    ...
    def _read_corpus_block_split(self, fname, block_size):
        ...
        while len(text) >= block_size:  # Truncate in block of block_size
            ...
            tokenized_text1 = self.tokenizer.convert_tokens_to_ids(
                self.tokenizer.tokenize(text[:block_size])
            )
            tokenized_text1 = self.tokenizer.add_special_tokens_single_sentence(
                tokenized_text1
            )
            ...
            tokenized_text1 = tokenized_text1 + (
                [pad_token_id] * (block_size - tokenized_text1_length - 2)
            )  # Pad up to the sequence length.
            assert len(tokenized_text1) == block_size
...
class TextDataset_2Tokenizers_LCtrlG(Dataset):
    def __init__(
        ...
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(
            directory, f"cached_lm_gpt_bert_{block_size}_{filename}"
        )
        ...
        # GPT tokenizers
        self.pad_token = tokenizers[1].convert_tokens_to_ids([tokenizers[1].pad_token])[
            0
        ]
        self.bos_token = tokenizers[1].convert_tokens_to_ids([tokenizers[1].bos_token])[
            0
        ]
        self.eos_token = tokenizers[1].convert_tokens_to_ids([tokenizers[1].eos_token])[
            0
        ]
        ...
        if not create_new and os.path.exists(cached_features_file):
            ...
            if text_split_mode == "natural":
                if args.dataset == "Yelp":
                    dropped = self._read_corpus_natural_split_yelp(
                        fname=file_path,
                        label=True,
                        max_length=block_size,
                        block_size=block_size,
    ...
    def __getitem__(self, item):
        ...
        tokenized_text_lengths = torch.tensor(
            [self.examples[item][1], self.examples[item][3]], dtype=torch.long
        )
    ...
    def _read_corpus_natural_split_yelp(self, fname, label, max_length, block_size):
        ...
        with open(fname) as fin, open(label_fname) as lfin:
            for line, label_line in zip(fin, lfin):
                ...
                tokenized_text0_length = len(tokenized_text0)
                pad_token = self.tokenizers[0].convert_tokens_to_ids(
                    [self.tokenizers[0].pad_token]
                )[0]
                # pad to max_seq_length (block_size)
                if block_size > tokenized_text0_length:
                    tokenized_text0 = tokenized_text0 + (
                        [pad_token] * (block_size - tokenized_text0_length)
                    )  # Pad up to the sequence length.
                else:
                    dropped += 1
                    continue
                ...
                if block_size > tokenized_text1_length:
                    tokenized_text1 = tokenized_text1 + (
                        [self.pad_token] * (block_size - tokenized_text1_length)
                    )  # Pad up to the sequence length.
                else:
                    dropped += 1
                    continue
                ...
                self.examples.append(
                    [
                        tokenized_text0,
                        tokenized_text0_length,
                        tokenized_text1,
                        tokenized_text1_length,
                        lb,
                    ]
                )
...
class TextDataset_2Tokenizers(Dataset):
    ...
    def _read_corpus_block_split(self, fname, block_size):
        ...
        while len(text) >= block_size:  # Truncate in block of block_size
            ...
            tokenized_text0 = self.tokenizers[0].convert_tokens_to_ids(
                self.tokenizers[0].tokenize(text[:block_size])
            )
            tokenized_text0 = self.tokenizers[0].add_special_tokens_single_sentence(
                tokenized_text0
            )
            ...
            tokenized_text1 = self.tokenizers[1].convert_tokens_to_ids(
                self.tokenizers[1].tokenize(text[:block_size])
            )
            tokenized_text1 = self.tokenizers[1].add_special_tokens_single_sentence(
                tokenized_text1
            )
            ...
            tokenized_text1 = tokenized_text1 + (
                [pad_token] * (block_size - tokenized_text1_length - 2)
            )  # Pad up to the sequence length.
            assert len(tokenized_text1) == block_size
...
def reconstruct(model, test_data_batch, vocab, strategy, fname):
    ...
    with open(fname, "w") as fout:
        ...
        for batch_data in test_data_batch:
            ...
            source = [
                [vocab.id2word(id_.item()) for id_ in sent] for sent in batch_data
            ]
...
def visualize_latent(args, epoch, vae, device, test_data):
    ...
    with open(os.path.join(args.exp_dir, f"synthetic_latent_{epoch}.txt"), "w") as f:
        test_data_batch, test_label_batch = test_data.create_data_batch_labels(
            ...
            for i in range(batch_size):
                for j in range(nsamples):
                    ...
                    f.write(
                        batch_label[i]
                        + "\t"
                        + " ".join([str(val) for val in sample])
                        + "\n"
                    )
